...
class dts_ConvAI2(object):
    ...
    def get_data(self, train=True, mode='none', revised=False, cands=False):
        ...
        assert mode in ['none', 'both', 'self', 'other']
        assert os.path.exists(txt_path) 
        return self._txt_to_json(txt_path, mode, cands)

    def get_dialogs(self, train=True):
        txt_path = os.path.join(
            self.path, '{}_none_original_no_cands.txt'.format(
            'train' if train is True else 'valid'))
        dialogs = [sample['dialog']
            for sample in self.get_data(train, 'none', False, False)]
        return dialogs
