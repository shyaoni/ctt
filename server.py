...
def add_message(uuid):
    if uuid == "init": #Create a new session
        ...
        sess[sessionID] = api.sess()

        if sessionID not in usr_stack[userID]:
    ...
    if uuid == "next": #Get your system's next response
        ...
        sess[sessionID].step('human: ' + context['text']) 
    ...
    if uuid == "end":  #Terminate a session with your system
        ...
        if sum(list(end_stack[userID].values())) >= 2:
